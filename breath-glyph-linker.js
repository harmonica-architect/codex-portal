...
class BreathGlyphLinker {
  ...
  _scheduleNext(seq, currentIdx) {
    clearTimeout(this.timer);
    // Breath phase durations (ms) — approximate to icositetragon timing
    const durations = {
      'inhale': 2200,
      'hold':   1600,
      'exhale': 2400,
    ...
    const dur = durations[seq[currentIdx].breath] || 2000;
    this.timer = setTimeout(() => {
      const nextIdx = (currentIdx + 1) % seq.length;
      this.currentStep = nextIdx;
      this._emitStep(seq[nextIdx], nextIdx, this.activeGlyph);
      this._scheduleNext(seq, nextIdx);
    }, dur);
  }
